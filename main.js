...
// =====================================================================
// SAYSFARM: DASHBOARD TERMINAL CON INTELIGENCIA DE AUTO-COMPRA ÓPTIMA
// MODIFICADO: PANEL DE BOLSA, TRIGGER GODZAMOK Y CONSOLA JS INTEGRADA
// =====================================================================
Game.farmModeActive = false;
Game.farmSavedPrefs = null;
Game.farmStartingCookiesEarned = 0; 
Game.farmStartingBank = 0;          
Game.farmSessionShimmers = 0;       
Game.farmEvents = [];               
Game.farmMilestones = null;         
Game.farmClickInterval = null;       
Game.farmWorker = null;             
Game.originalPlayCookieClickSound = null; 
Game.farmGodzamokCooldown = false;

// Variables para el cronómetro de la sesión
Game.farmStartTime = 0;

// Sistema de temporizadores virtuales
Game.farmFakeBuffs = [];

// Variables para el cálculo dinámico de clics por segundo
Game.farmClickCounter = 0;          
Game.farmClicksInLastSecond = "0.0";   
Game.farmLastCpsUpdate = 0;         

// --- FUNCIONES AUXILIARES DE TRADING Y MACROS ---
Game.TerminalBuyBTR = function() {
    var btr = Game.Objects['Bank'];
    if (btr && btr.minigame) {
        var marketBTR = btr.minigame.goodsById[2];
        if (marketBTR) {
            var maxBuy = btr.minigame.getGoodMaxStock(marketBTR) - marketBTR.stock;
            if (maxBuy > 0) {
                btr.minigame.buyGood(2, maxBuy);
                Game.LogFarmEvent("📊 Bolsa: Has comprado " + maxBuy + " acciones de BTR desde la terminal.");
            } else {
                Game.LogFarmEvent("⚠️ Bolsa: El almacén de BTR ya está lleno.");
            }
        }
    } else {
        Game.LogFarmEvent("❌ Error: El minijuego de la Bolsa no está activo.");
    }
};

Game.TerminalSellAll = function() {
    var bankObj = Game.Objects['Bank'];
    if (bankObj && bankObj.minigame) {
        var mg = bankObj.minigame;
        var soldAny = false;
        for (var i = 0; i < mg.goodsById.length; i++) {
            var good = mg.goodsById[i];
            if (good.stock > 0) {
                mg.sellGood(good.id, good.stock);
                soldAny = true;
            }
        }
        if (soldAny) Game.LogFarmEvent("💰 Bolsa: ¡Has liquidado TODAS tus acciones con éxito!");
        else Game.LogFarmEvent("⚠️ Bolsa: No tienes acciones en cartera para vender.");
    } else {
        Game.LogFarmEvent("❌ Error: El minijuego de la Bolsa no está activo.");
    }
};

Game.TerminalTriggerGodzamok = function() {
    var cursorObj = Game.Objects['Cursor'];
    if (cursorObj && cursorObj.amount >= 100) {
        var toSell = 100;
        cursorObj.sell(toSell);
        Game.LogFarmEvent("🔥 Godzamok: Sacrificados " + toSell + " cursores. Bufo de clics activado.");
        
        setTimeout(function() {
            cursorObj.buy(toSell);
            Game.LogFarmEvent("🤖 Auto-Compra: Repuestos los " + toSell + " cursores del sacrificio.");
        }, 10000);
    } else {
        Game.LogFarmEvent("⚠️ Godzamok: No tienes suficientes cursores (mínimo 100).");
    }
};

Game.TerminalExecuteJS = function() {
    var input = document.getElementById('terminalConsoleInput');
    if (input && input.value.trim() !== "") {
        var code = input.value;
        try {
            var result = eval(code);
            Game.LogFarmEvent("💻 JS Exec: " + code + " ➔ " + result);
            input.value = "";
        } catch (err) {
            Game.LogFarmEvent("🔴 JS Error: " + err.message);
        }
    }
};

// Formateador de números grandes
var formatFullNumber = function(val, hasDecimals) {
    if (!isFinite(val)) return 'Infinity';
    if (val >= 1e15) {
...
// Registro de eventos en la consola
Game.LogFarmEvent = function(text) {
    ...
    if (cleanText.toLowerCase().indexOf('lucky') !== -1 || cleanText.toLowerCase().indexOf('afortunado') !== -1) {
        formattedText = '<span style="color: #ffeb3b; font-weight: bold;">★ ' + cleanText + ' ★</span>';
    } else if (cleanText.indexOf('★ HITO') !== -1) {
        formattedText = '<span style="color: #00e676; font-weight: bold;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('🏆 ¡LOGRO') !== -1) {
        formattedText = '<span style="color: #00e5ff; font-weight: bold; background: rgba(0, 229, 255, 0.15); padding: 2px 6px; border-radius: 4px; border: 1px solid #00e5ff; display: inline-block; margin: 3px 0; box-shadow: 0 0 8px rgba(0,229,255,0.3);">★ ' + cleanText + ' ★</span>';
    } else if (cleanText.indexOf('💾 Autoguardado') !== -1) {
        formattedText = '<span style="color: #90caf9;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('🤖 Auto-') !== -1) {
        formattedText = '<span style="color: #b0bec5; font-style: italic;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('📊 Bolsa') !== -1 || cleanText.indexOf('💰 Bolsa') !== -1) {
        formattedText = '<span style="color: #ffb74d; font-weight: bold;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('🔮 Auto-Hechizo') !== -1) {
        formattedText = '<span style="color: #81d4fa; font-weight: bold;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('🔥 Godzamok') !== -1 || cleanText.indexOf('🔥 Auto-Godzamok') !== -1) {
        formattedText = '<span style="color: #ff5252; font-weight: bold; text-shadow: 0 0 4px #ff5252;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('💻 JS Exec') !== -1) {
        formattedText = '<span style="color: #e040fb;">' + cleanText + '</span>';
    } else if (cleanText.indexOf('🔴 JS Error') !== -1) {
        formattedText = '<span style="color: #ff1744; font-weight: bold; background: rgba(255,23,68,0.1); padding: 1px 4px;">' + cleanText + '</span>';
    }
    
    Game.farmEvents.unshift("[" + timeStr + "] " + formattedText);
    if (Game.farmEvents.length > 50) Game.farmEvents.pop();
    
    var eventsEl = document.getElementById('farmEvents');
    if (eventsEl) {
        eventsEl.innerHTML = Game.farmEvents.join('<br>');
    }
};

Game.ExitFarmMode = function() {
    if (Game.farmWorker) {
        Game.farmWorker.postMessage('stop');
        Game.farmWorker.terminate();
        Game.farmWorker = null;
    }
    if (Game.farmClickInterval) {
        clearInterval(Game.farmClickInterval);
        Game.farmClickInterval = null;
    }

    var input = document.getElementById('farmRenameInput');
    var newName = input ? input.value : "";
    
    if (newName.length === 0 || newName.toLowerCase() === 'saysfarm') {
        newName = Game.RandomBakeryName();
    }
    
    Game.bakeryNameSet(newName);
    Game.farmModeActive = false;
    
    var overlay = document.getElementById('saysfarm-overlay');
    if (overlay) overlay.parentNode.removeChild(overlay);
    
    var cols = ['sectionLeft', 'sectionMiddle', 'sectionRight'];
    for (var i = 0; i < cols.length; i++) {
        var el = document.getElementById(cols[i]);
        if (el) el.style.display = '';
    }
    
    if (Game.farmSavedPrefs) {
        Game.prefs.particles = Game.farmSavedPrefs.particles;
        Game.prefs.fancy = Game.farmSavedPrefs.fancy;
        Game.prefs.filters = Game.farmSavedPrefs.filters;
        Game.prefs.wobbly = Game.farmSavedPrefs.wobbly;
        Game.prefs.focus = Game.farmSavedPrefs.focus;
        Game.fps = Game.farmSavedPrefs.fps;
    } else {
        Game.fps = 30;
    }

    if (typeof Game.originalPlayCookieClickSound === 'function') {
        Game.playCookieClickSound = Game.originalPlayCookieClickSound;
    }

    if (Game.prefs.fancy) Game.removeClass('noFancy'); else Game.addClass('noFancy');
    if (Game.prefs.filters) Game.removeClass('noFilters'); else Game.addClass('noFilters');
    Game.ToggleFancy();
    Game.ToggleFilters();
    Game.storeToRefresh = 1;
    Game.upgradesToRebuild = 1;
    
    console.log("Modo Ahorro desactivado. Interfaz visual restaurada.");
};

Game.EnableFarmMode = function() {
    Game.farmStartingCookiesEarned = Game.cookiesEarned;
    Game.farmStartingBank = Game.cookies; 
    Game.farmSessionShimmers = 0;
    Game.farmEvents = []; 
    Game.farmFakeBuffs = [];
    Game.farmMilestones = {10:false, 25:false, 50:false, 100:false, 200:false, 500:false, 1000:false};
    Game.farmStartTime = Date.now();

    Game.farmClickCounter = 0;
    Game.farmClicksInLastSecond = "0.0";
    Game.farmLastCpsUpdate = Date.now();

    Game.farmSavedPrefs = {
        particles: Game.prefs.particles,
        fancy: Game.prefs.fancy,
        filters: Game.prefs.filters,
        wobbly: Game.prefs.wobbly,
        numbers: Game.prefs.numbers,
        focus: Game.prefs.focus,
        fps: Game.fps
    };

    Game.farmModeActive = true;
    Game.fps = 60;

    console.log("Modo ahorro activado. Suspendiendo gráficos...");
    ...
    if (typeof Game.playCookieClickSound === 'function') {
        Game.originalPlayCookieClickSound = Game.playCookieClickSound;
        Game.playCookieClickSound = function() {}; 
    }
    ...
    if (Game.shimmers && Game.shimmers.length > 0) {
        for (var i = Game.shimmers.length - 1; i >= 0; i--) {
            ...
            if (shimmer && typeof shimmer.pop === 'function') {
                shimmer.pop();
                Game.farmSessionShimmers++;
            }
        } 
    }
    ...
    if (!document.getElementById('saysfarm-overlay')) {
        var overlay = document.createElement('div');
        overlay.id = 'saysfarm-overlay';
        overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:#000;z-index:99999999;color:#fff;display:flex;flex-direction:column;justify-content:center;align-items:center;font-family:monospace;text-align:center;line-height:1.4;overflow-y:auto;padding:20px 0;';
        
        var html = '';
        html += '<h1 style="font-size:22px;color:#00ffcc;margin-bottom:15px;text-transform:uppercase;letter-spacing:2px;">Saysfarm - Modo Terminal</h1>';
        
        html += '<div style="font-size:14px;color:#888;">Galletas en el banco:</div>';
        html += '<div id="farmCookies" style="font-size:32px;color:#ffcc00;font-weight:bold;margin-bottom:10px;word-break:break-all;max-width:90%;">0</div>';
        
        html += '<div style="font-size:14px;color:#888;">Galletas de la sesión:</div>';
        html += '<div id="farmedSession" style="font-size:32px;color:#00ffcc;font-weight:bold;margin-bottom:10px;word-break:break-all;max-width:90%;">0</div>';

        html += '<div style="display:flex;gap:20px;margin-bottom:10px;">';
        html += '  <div><div style="font-size:12px;color:#888;">Crecimiento:</div><div id="farmPct" style="font-size:16px;color:#00ff55;font-weight:bold;">+0.00%</div></div>';
        html += '  <div><div style="font-size:12px;color:#888;">Tiempo:</div><div id="farmTime" style="font-size:16px;color:#fff;font-weight:bold;">00:00:00</div></div>';
        html += '  <div><div style="font-size:12px;color:#888;">Clics:</div><div id="farmCookieCps" style="font-size:16px;color:#00ffcc;font-weight:bold;">0.0 Cps</div></div>';
        html += '</div>';

        html += '<div style="font-size:14px;color:#888;">Producción real: <span id="farmCps" style="color:#aaccff;font-weight:bold;">0 /s</span></div>';
        html += '<div style="font-size:13px;color:#888;margin-top:4px;">Efectos activos: <span id="farmBuffs" style="color:#ff00ff;font-weight:bold;">Ninguno</span></div>';
        html += '<div style="font-size:13px;color:#888;margin-top:4px;">Siguiente objetivo: <span id="farmAutoBuyer" style="color:#00ffcc;font-weight:bold;">Calculando...</span></div>';
        html += '<div style="font-size:13px;color:#888;margin-top:4px;margin-bottom:15px;">Doras/Renos cazados: <span id="farmShimmerCount" style="color:#ff9900;font-weight:bold;">0</span></div>';

        html += '<div style="background:#0a0a0a;border:1px solid #222;padding:12px;border-radius:6px;width:420px;margin-bottom:15px;text-align:left;box-sizing:border-box;font-size:12px;">';
        html += '  <div style="color:#00ffcc;font-weight:bold;font-size:12px;margin-bottom:8px;text-transform:uppercase;letter-spacing:1px;">⚙️ Automatizaciones Activas</div>';
        html += '  <label style="display:flex;align-items:center;margin-bottom:6px;cursor:pointer;"><input type="checkbox" id="chkAutoBuyer" checked style="margin-right:8px;cursor:pointer;"> Auto-Comprar Edificios y Mejoras</label>';
        html += '  <label style="display:flex;align-items:center;margin-bottom:6px;cursor:pointer;"><input type="checkbox" id="chkAutoStock" checked style="margin-right:8px;cursor:pointer;"> Auto-Trading de Acciones (Bolsa)</label>';
        html += '  <label style="display:flex;align-items:center;margin-bottom:6px;cursor:pointer;"><input type="checkbox" id="chkAutoGodzamok" checked style="margin-right:8px;cursor:pointer;"> Auto-Godzamok (Venta Inteligente de Cursores)</label>';
        html += '  <label style="display:flex;align-items:center;margin-bottom:6px;cursor:pointer;"><input type="checkbox" id="chkAutoSpells" checked style="margin-right:8px;cursor:pointer;"> Auto-Hechizos en Frenesí (Grimorio)</label>';
        html += '</div>';

        html += '<div style="background:#0a0a0a;border:1px solid #222;padding:12px;border-radius:6px;width:420px;margin-bottom:15px;text-align:left;box-sizing:border-box;">';
        html += '  <div style="color:#ffb74d;font-weight:bold;font-size:12px;margin-bottom:8px;text-transform:uppercase;">📊 Panel Financiero & Macros Manuales</div>';
        html += '  <div style="display:flex;gap:10px;margin-bottom:8px;">';
        html += '    <button onclick="Game.TerminalBuyBTR()" style="flex:1;background:#e65100;color:#fff;border:none;padding:6px;font-family:monospace;font-weight:bold;cursor:pointer;font-size:11px;border-radius:3px;">[ COMPRAR BTR MAX ]</button>';
        html += '    <button onclick="Game.TerminalSellAll()" style="flex:1;background:#37474f;color:#fff;border:none;padding:6px;font-family:monospace;font-weight:bold;cursor:pointer;font-size:11px;border-radius:3px;">[ VENDER TODO ALL ]</button>';
        html += '  </div>';
        html += '  <button onclick="Game.TerminalTriggerGodzamok()" style="width:100%;background:#b71c1c;color:#fff;border:none;padding:7px;font-family:monospace;font-weight:bold;cursor:pointer;font-size:12px;border-radius:3px;box-shadow:0 0 6px rgba(183,28,28,0.4);">💥 FORZAR SACRIFICIO DE 100 CURSORES (MANUAL)</button>';
        html += '</div>';

        html += '<div style="background:#0a0a0a;border:1px solid #222;padding:12px;border-radius:6px;width:420px;margin-bottom:15px;text-align:left;box-sizing:border-box;">';
        html += '  <div style="color:#e040fb;font-weight:bold;font-size:12px;margin-bottom:6px;text-transform:uppercase;">💻 Consola JS Integrada</div>';
        html += '  <div style="display:flex;gap:6px;">';
        html += '    <input type="text" id="terminalConsoleInput" placeholder="Ej: Game.cookies += 100000;" style="flex:1;background:#111;color:#fff;border:1px solid #333;padding:6px;font-family:monospace;font-size:12px;border-radius:3px;outline:none;">';
        html += '    <button onclick="Game.TerminalExecuteJS()" style="background:#7b1fa2;color:#fff;border:none;padding:0 12px;font-family:monospace;font-weight:bold;cursor:pointer;font-size:12px;border-radius:3px;">RUN</button>';
        html += '  </div>';
        html += '</div>';

        html += '<div style="font-size:13px;color:#888;margin-bottom:4px;text-align:left;width:420px;">Registro de actividad:</div>';
        html += '<div id="farmEvents" style="font-size:11px;color:#00ff00;height:110px;overflow-y:scroll;background:#0d0d0d;padding:10px;border-radius:4px;border:1px solid #1a1a1a;width:420px;text-align:left;box-sizing:border-box;scrollbar-width:thin;">';
        html += '    <span style="color:#555;">Esperando eventos de juego...</span>';
        html += '</div>';
        
        html += '<div style="margin-top:15px;background:#111;padding:12px;border:1px solid #333;border-radius:8px;text-align:left;width:320px;box-sizing:border-box;">';
        html += '    <input type="text" id="farmRenameInput" placeholder="Nuevo nombre de pastelería" style="padding:8px;font-size:14px;text-align:center;background:#222;color:#fff;border:1px solid #444;border-radius:4px;width:100%;margin-bottom:8px;display:block;outline:none;box-sizing:border-box;">';
        html += '    <button onclick="Game.ExitFarmMode()" style="padding:8px 20px;font-size:14px;background:#00ffcc;color:#000;border:none;border-radius:4px;cursor:pointer;font-weight:bold;width:100%;box-shadow:0 4px 6px rgba(0,255,204,0.2);">Volver al Juego</button>';
        html += '</div>';
        
        overlay.innerHTML = html;
        document.body.appendChild(overlay);

        document.getElementById('terminalConsoleInput').addEventListener('keydown', function(e) {
            if (e.key === 'Enter') Game.TerminalExecuteJS();
        });
    }

    if (window.Worker) {
        try {
            var blobCode = "var interval = null;\n" +
                           "onmessage = function(e) {\n" +
                           "   if (e.data === 'start') {\n" +
                           "       interval = setInterval(function() { postMessage('tick'); }, 20);\n" +
                           "   } else if (e.data === 'stop') {\n" +
                           "       if (interval) clearInterval(interval);\n" +
                           "   }\n" +
                           "};";
            var blob = new Blob([blobCode], { type: 'text/javascript' });
            Game.farmWorker = new Worker(URL.createObjectURL(blob));
            
            Game.farmWorker.onmessage = function(e) {
                if (Game.farmModeActive) {
                    Game.ClickCookie();
                    Game.farmClickCounter++;
                }
            };
            Game.farmWorker.postMessage('start');
        } catch (err) {
            Game.farmClickInterval = setInterval(function() {
                Game.ClickCookie();
                Game.farmClickCounter++;
            }, 20);
        }
...
Game.RunAutoAutomations = function() {
    if (document.getElementById('chkAutoBuyer') && document.getElementById('chkAutoBuyer').checked) {
        ...
        for (var i in Game.Objects) {
            ...
            var cost = b.getPrice();
            var addedCps = b.storedCps * Game.globalCpsMult;
            if (addedCps > 0) {
                var eff = cost / addedCps;
                if (eff < bestEff) {
                    bestEff = eff;
                    bestBuilding = b;
                }
        ...
        var abEl = document.getElementById('farmAutoBuyer');
        if (abEl) {
            if (bestBuilding) {
                abEl.innerHTML = '<span style="color:#00ffcc;">' + bestBuilding.name + '</span> <span style="color:#888;">(' + formatFullNumber(bestBuilding.getPrice(), false) + ' galletas)</span>';
            } else {
                abEl.textContent = 'Calculando...';
            }
        }

        if (bestBuilding && Game.cookies >= bestBuilding.getPrice() && bestBuilding.getPrice() < Game.cookies * 0.35) {
            var bPrice = bestBuilding.getPrice();
            bestBuilding.buy(1);
            Game.LogFarmEvent("🤖 Auto-Compra: Se compró 1 " + bestBuilding.single + " por " + formatFullNumber(bPrice, false) + " galletas (Amortización: " + formatFullNumber(bestEff, false) + "s).");
        }

        if (Game.UpgradesInStore && Game.UpgradesInStore.length > 0) {
            for (var k = 0; k < Game.UpgradesInStore.length; k++) {
                var u = Game.UpgradesInStore[k];
                if (u) {
                    var uCost = u.getPrice();
                    if (Game.cookies >= uCost && uCost < Game.cookies * 0.15 && u.pool !== 'toggle' && u.pool !== 'tech') {
                        u.buy(1);
                        Game.LogFarmEvent("🤖 Auto-Compra: Se adquirió la mejora '" + u.name + "' por " + formatFullNumber(uCost, false) + " galletas.");
                        break; 
                    }
                }
            }
    ...
    if (document.getElementById('chkAutoStock') && document.getElementById('chkAutoStock').checked) {
        var bankObj = Game.Objects['Bank'];
        if (Game.isMinigameReady(bankObj)) {
            var mg = bankObj.minigame;
            for (var i = 0; i < mg.goodsById.length; i++) {
                var good = mg.goodsById[i];
                var price = good.val;
                var refPrice = 10 + good.id * 10 + (bankObj.level - 1);
                
                if (price < refPrice * 0.40) {
                    var maxBuy = mg.getGoodMaxStock(good) - good.stock;
                    if (maxBuy > 0) {
                        var costInCookies = price * maxBuy * Game.cookiesPsRaw;
                        if (Game.cookies >= costInCookies) {
                            mg.buyGood(good.id, maxBuy);
                            Game.LogFarmEvent("📊 Bolsa Auto: Adquiridas " + maxBuy + " acciones de " + good.name + " a un precio bajo de $" + price.toFixed(2) + ".");
                        }
                    }
                }
                else if (price > refPrice * 1.35 && good.stock > 0) {
                    var stock = good.stock;
                    mg.sellGood(good.id, stock);
                    Game.LogFarmEvent("💰 Bolsa Auto: Vendidas " + stock + " acciones de " + good.name + " a un precio pico de $" + price.toFixed(2) + " (Alta ganancia).");
                }
            }
        }
    }
...
// --- INTERCEPCIÓN E INTEGRACIÓN EN EL NÚCLEO DEL JUEGO (Game.Init) ---
// Sobrescribimos temporalmente Game.Launch para interceptar las funciones en el momento adecuado
var originalLaunch = Game.Launch;
Game.Launch = function() {
    originalLaunch(); // Ejecutamos la función original de Cookie Clicker (define Game.Init, Game.registerMod, etc.)

    // 1. Capturar ventanas emergentes (Game.Popup)
    ...
    // 2. Capturar logros nuevos (Game.Win)
    var originalWin = Game.Win;
    Game.Win = function(what) {
        if (typeof what === 'string') {
            var achiev = Game.Achievements[what];
            if (achiev && achiev.won == 0 && Game.farmModeActive) {
    ...
    // 3. Capturar autoguardados del sistema
    var originalWriteSave = Game.WriteSave;
    Game.WriteSave = function(type) {
        var res = originalWriteSave(type);
        if (Game.farmModeActive && !type) {
            Game.LogFarmEvent("💾 Autoguardado: Progreso salvado en el almacenamiento local.");
        }
        return res;
    };

    // 4. Desviar el dibujado gráfico de forma limpia para ahorrar el 100% de GPU/CPU de renderizado
    var originalDraw = Game.Draw;
    Game.Draw = function() {
        if (Game.farmModeActive) {
            var cookiesEl = document.getElementById('farmCookies');
            var farmedEl = document.getElementById('farmedSession');
            var pctEl = document.getElementById('farmPct');
            var cpsEl = document.getElementById('farmCps');
            var buffsEl = document.getElementById('farmBuffs');
            var shimmersEl = document.getElementById('farmShimmerCount');
            var cookieCpsEl = document.getElementById('farmCookieCps');
            var timeEl = document.getElementById('farmTime');
            
            var now = Date.now();
            var elapsed = now - Game.farmLastCpsUpdate;
            if (elapsed >= 1000) {
                Game.farmClicksInLastSecond = ((Game.farmClickCounter / elapsed) * 1000).toFixed(1);
                Game.farmClickCounter = 0;
                Game.farmLastCpsUpdate = now;
            }

            if (timeEl && Game.farmStartTime > 0) {
                var elapsedMs = now - Game.farmStartTime;
                ...
                timeEl.textContent = padZero(hr) + ":" + padZero(min) + ":" + padZero(sec);
            }

            var farmedSinceStart = Math.max(0, Game.cookiesEarned - Game.farmStartingCookiesEarned);
            var startingBank = Game.farmStartingBank || Game.cookies;
            var pctIncrease = startingBank > 0 ? (farmedSinceStart / startingBank) * 100 : 0;
            
            var milestones = [10, 25, 50, 100, 200, 500, 1000];
            for (var i = 0; i < milestones.length; i++) {
                var m = milestones[i];
                if (pctIncrease >= m && Game.farmMilestones && !Game.farmMilestones[m]) {
                    Game.farmMilestones[m] = true;
                    Game.LogFarmEvent("★ HITO ALCANZADO: ¡Has producido el " + m + "% de tu banco inicial (" + formatFullNumber(startingBank, false) + " galletas)!");
                }
            }

            if (cookiesEl) cookiesEl.textContent = formatFullNumber(Game.cookies, false);
            if (farmedEl) farmedEl.textContent = formatFullNumber(farmedSinceStart, false);
            if (pctEl) pctEl.textContent = "+" + formatFullNumber(pctIncrease, true) + "%";
            if (cpsEl) cpsEl.textContent = formatFullNumber(Game.cookiesPs * (1 - Game.cpsSucked), true) + " /s";
            if (shimmersEl) shimmersEl.textContent = formatFullNumber(Game.farmSessionShimmers, false);
            if (cookieCpsEl) cookieCpsEl.textContent = Game.farmClicksInLastSecond + " Cps";
            ...
            if (buffsEl) {
                ...
                for (var i in Game.buffs) {
                    var buff = Game.buffs[i];
                    if (buff && buff.time > 0) {
                        var remainingSecs = Math.max(0, buff.time / Game.fps).toFixed(1);
                        activeBuffs.push(buff.name + " (" + remainingSecs + "s)");
                    }
                }
                if (Game.farmFakeBuffs) {
                    for (var i = 0; i < Game.farmFakeBuffs.length; i++) { 
                        var fakeBuff = Game.farmFakeBuffs[i];
                        var remainingSecs = Math.max(0, fakeBuff.time / Game.fps).toFixed(1);
                        activeBuffs.push(fakeBuff.name + " (" + remainingSecs + "s)");
                    }
                }
                if (activeBuffs.length > 0) {
                    buffsEl.textContent = activeBuffs.join(' | ');
                    buffsEl.style.color = '#ff33ff';
                } else {
                    buffsEl.textContent = 'Ninguno';
                    buffsEl.style.color = '#555';
                }
            }
            return; 
        }
    ...
    // 5. Registrar el mod mediante la API oficial
    Game.registerMod("saysfarm", {
        init: function() {
            Game.LogFarmEvent("🤖 Mod 'saysfarm' registrado correctamente. Esperando activación por nombre...");
            
            // Añadimos el gancho de lógica del juego (Logic Loop Hook)
            Game.registerHook('logic', function() {
                if (Game.bakeryName.toLowerCase() === "saysfarm" && !Game.farmModeActive) {
                    Game.EnableFarmMode();
                }
                
                if (Game.farmModeActive) {
                    // Auto-Cazar shimmers instantáneamente (Galletas Doradas y Renos)
                    if (Game.shimmers && Game.shimmers.length > 0) {
                        for (var i = Game.shimmers.length - 1; i >= 0; i--) {
                            var shimmer = Game.shimmers[i];
                            if (shimmer && typeof shimmer.pop === 'function') {
                    ...
                    // Actualizar temporizadores de efectos virtuales
                    if (Game.farmFakeBuffs) {
                        for (var i = Game.farmFakeBuffs.length - 1; i >= 0; i--) {
                            Game.farmFakeBuffs[i].time--;
                            if (Game.farmFakeBuffs[i].time <= 0) {
                                Game.farmFakeBuffs.splice(i, 1);
                            }
                        }
                    }

                    // A. Auto-Godzamok Inteligente (Venta durante bufos de clics)
                    var chkGodzamok = document.getElementById('chkAutoGodzamok');
                    if (chkGodzamok && chkGodzamok.checked) {
                        if (Game.hasGod && Game.hasGod('ruin')) { // Si Godzamok está activo en el Panteón
                            if (Game.hasBuff('Click frenzy') || Game.hasBuff('Dragonflight')) {
                                var cursorObj = Game.Objects['Cursor'];
                                if (cursorObj && cursorObj.amount >= 100 && !Game.farmGodzamokCooldown) {
                                    var toSell = Math.min(cursorObj.amount - 10, 100); // Guardar un margen mínimo de 10 cursores
                                    if (toSell >= 50) {
                                        cursorObj.sell(toSell);
                                        Game.LogFarmEvent("🔥 Auto-Godzamok: ¡Sacrificados " + toSell + " cursores durante un bufo de clics!");
                                        Game.farmGodzamokCooldown = true;
                                        
                                        setTimeout(function() {
                                            cursorObj.buy(toSell);
                                            Game.LogFarmEvent("🤖 Auto-Godzamok: Recomprados los " + toSell + " cursores.");
                                            Game.farmGodzamokCooldown = false;
                                        }, 10000);
                                    }
                                }
                    ...
                    // B. Auto-Hechizos en Frenesí (Grimorio)
                    var chkSpells = document.getElementById('chkAutoSpells');
                    if (chkSpells && chkSpells.checked) {
                        var wizardObj = Game.Objects['Wizard tower'];
                        if (wizardObj && wizardObj.minigameLoaded && wizardObj.minigame) {
                            var minigame = wizardObj.minigame;
                            // Si la magia está por encima del 95% de su capacidad y tenemos el bufo de Frenesí activo
                            if (minigame.magic >= minigame.magicM * 0.95 && Game.hasBuff('Frenzy')) {
                                var handOfFate = minigame.spellsById[2]; // ID 2 es 'Force the Hand of Fate'
                                var spellCost = minigame.getSpellCost(handOfFate);
                                if (minigame.magic >= spellCost) {
                                    minigame.castSpell(handOfFate);
                                    Game.LogFarmEvent("🔮 Auto-Hechizo: Lanzado 'Force the Hand of Fate' para acumular multiplicadores.");
                                }
                            }
                        }
                    }

                    // C. Automatizaciones periódicas cada segundo (60 frames)
                    if (Game.T % 60 === 0) {
                        Game.RunAutoAutomations();
                    }
                }
            });
        }
